...
# test SNR
# calculate SNR
signal_in = df_as_85_cvs_r_a

num_peak = 7
snr_numerator= 0
snr_denominator= 0
for j in range (num_peak):
    snr_numerator = snr_numerator + (signal_in.iloc[0, 1000*j+850:1000*j+1150]**2).sum()
    snr_denominator = snr_denominator + (signal_in.iloc[0, 1000*j+150:1000*j+850]**2).sum()
snr_numerator = (snr_numerator - (signal_in.iloc[0, 1000*num_peak:1000*num_peak+150]**2).sum())/(300*(num_peak-1)+150)
snr_denominator = (snr_denominator + (signal_in.iloc[0, 0:150]**2).sum())/(700*(num_peak-1)+850)


test_snr = snr_numerator / snr_denominator
test_sum2 = (signal_in.iloc[0, 0:7000]**2).sum()
...
fig_sameday_as_SNR_in_1(df_as_85_cvs_r_a, title = 'SNR for 85dB a vowel retest subjects in frequency domain')
fig_sameday_as_SNR_in_1(df_as_85_cvs_r_e, title = 'SNR for 85dB e vowel retest subjects in frequency domain')
fig_sameday_as_SNR_in_1(df_as_85_cvs_r_n, title = 'SNR for 85dB n vowel retest subjects in frequency domain')
fig_sameday_as_SNR_in_1(df_as_85_cvs_r_u, title = 'SNR for 85dB u vowel retest subjects in frequency domain')

fig_sameday_as_SNR_in_1(df_as_85_cvs_t_a, title = 'SNR for 85dB a vowel test subjects in frequency domain')
fig_sameday_as_SNR_in_1(df_as_85_cvs_t_e, title = 'SNR for 85dB e vowel test subjects in frequency domain')
fig_sameday_as_SNR_in_1(df_as_85_cvs_t_n, title = 'SNR for 85dB n vowel test subjects in frequency domain')
fig_sameday_as_SNR_in_1(df_as_85_cvs_t_u, title = 'SNR for 85dB u vowel test subjects in frequency domain')

fig_sameday_as_SNR_in_1_avg_8(df_as_85_cvs, title = 'SNR for 85dB subjects in frequency domain')
'''
