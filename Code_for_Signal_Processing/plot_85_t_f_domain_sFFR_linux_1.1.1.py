...
for i in range(4):
    ...
    temp = pd.concat([df_FFR_avg_t, df_FFR_avg_label], axis=1, ignore_index=True)
    df_FFR_avg_85_aenu = df_FFR_avg_85_aenu.append(temp, ignore_index=True)

    df_FFR_avg_win_t = pd.DataFrame(df_FFR_avg_win_85.iloc[44*i: 44*i+44, 0:1024].mean(axis=0).values.reshape(1,1024)) # average those two rows
    df_FFR_avg_win_label = pd.DataFrame(df_FFR_avg_win_85.iloc[44*i, 1024:1031].values.reshape(1,7))
    temp_win = pd.concat([df_FFR_avg_win_t, df_FFR_avg_win_label], axis=1, ignore_index=True)
    df_FFR_avg_win_85_aenu = df_FFR_avg_win_85_aenu.append(temp_win, ignore_index=True)

# set the title of columns
df_FFR_avg_85_aenu.columns = np.append(np.arange(1024), ["Subject", "Sex", "Condition", "Vowel", "Sound Level", "Num", "EFR/FFR"])
df_FFR_avg_85_aenu = df_FFR_avg_85_aenu.sort_values(by=["Condition", "Subject"])

df_FFR_avg_win_85_aenu.columns = np.append(np.arange(1024), ["Subject", "Sex", "Condition", "Vowel", "Sound Level", "Num", "EFR/FFR"])
df_FFR_avg_win_85_aenu = df_FFR_avg_win_85_aenu.sort_values(by=["Condition", "Subject"])
...
# for df_FFR_avg_85
df_FFR_avg_85_aenu_data = df_FFR_avg_85_aenu.iloc[:, :1024]
df_FFR_avg_85_aenu_label = df_FFR_avg_85_aenu.iloc[:, 1024:]

# for df_FFR_avg_win_85
df_FFR_avg_win_85_aenu_data = df_FFR_avg_win_85_aenu.iloc[:, :1024]
df_FFR_avg_win_85_aenu_label = df_FFR_avg_win_85_aenu.iloc[:, 1024:]
...
# for df_FFR_avg_aenu_withzero
df_FFR_win_85_aenu_as= pd.DataFrame()
df_FFR_win_85_aenu_as7= pd.DataFrame()
for i in range(4):
    temp_aenu_as = 2/n2 * np.abs((np.fft.fft(df_FFR_avg_win_85_aenu_data.iloc[i, :], n=96060))[range(int(n2/2))])
    df_FFR_win_85_aenu_as = df_FFR_win_85_aenu_as.append(pd.DataFrame(temp_aenu_as.reshape(1,48030)), ignore_index = True)
    df_FFR_win_85_aenu_as7 = df_FFR_win_85_aenu_as7.append(pd.DataFrame(np.array([temp_aenu_as[1000], temp_aenu_as[2000], temp_aenu_as[3000], temp_aenu_as[4000], \
                                                                  temp_aenu_as[5000], temp_aenu_as[6000], temp_aenu_as[7000]]).reshape(1,7)), ignore_index = True)

df_FFR_win_85_aenu_as = pd.concat([df_FFR_win_85_aenu_as, df_FFR_avg_85_aenu_label], axis=1) # add labels on it
df_FFR_win_85_aenu_as7 = pd.concat([df_FFR_win_85_aenu_as7, df_FFR_avg_85_aenu_label], axis=1) # add labels on it
...
plt.plot(x_label, np.asarray(df_FFR_avg_85_aenu.iloc[0,:1024]))
plt.plot(x_label, np.asarray(df_FFR_avg_85_aenu.iloc[1,:1024]))
plt.plot(x_label, np.asarray(df_FFR_avg_85_aenu.iloc[2,:1024]))
plt.plot(x_label, np.asarray(df_FFR_avg_85_aenu.iloc[3,:1024]))
plt.title('grand mean 85dB 4 vowel spectral FFRs in time domain')
...
plt.plot(x_label, df_FFR_win_85_aenu_as.iloc[0, :48030], '-')
plt.plot(x_label, df_FFR_win_85_aenu_as.iloc[1, :48030], '-')
plt.plot(x_label, df_FFR_win_85_aenu_as.iloc[2, :48030], '-')
plt.plot(x_label, df_FFR_win_85_aenu_as.iloc[3, :48030], '-')
plt.title('grand mean 85dB 4 vowel spectral FFRs in frequency domain')
plt.legend(('a', 'e', 'n', 'u'), loc='upper right')
...
plt.figure()
#markers = [100, 200, 300, 400, 500, 600, 700] # which corresponds to 100 200....700Hz in frequency domain
plt.plot(signal.resample(df_FFR_85_aenu_as.iloc[0, :48030], 48078), '-')
plt.plot(signal.resample(df_FFR_85_aenu_as.iloc[1, :48030], 47839), '-')
plt.plot(signal.resample(df_FFR_85_aenu_as.iloc[2, :48030], 47934), '-')
plt.plot(signal.resample(df_FFR_85_aenu_as.iloc[3, :48030], 48466), '-')
plt.title('resampled grand mean 85dB 4 vowel spectral FFRs in frequency domain')
plt.xlim(0,10000)
plt.legend(('a', 'e', 'n', 'u'), loc='upper right')
plt.show()
'''
...
# a vowel
axes[0, 0].plot(x_label_time, np.asarray(df_FFR_avg_85_aenu.iloc[0,:1024]))
axes[0, 0].set_xticks([])
axes[0, 0].set_ylabel(r'$Amplitude\  (\mu V) $')
axes[0, 1].plot(x_label_freq, df_FFR_win_85_aenu_as.iloc[0, :48030], label="a vowel") 
axes[0, 1].set_xlim(0, 1300)
axes[0, 1].legend(loc="upper right")

# e vowel
axes[1, 0].plot(x_label_time, np.asarray(df_FFR_avg_85_aenu.iloc[1,:1024]))
axes[1, 0].set_xticks([])
axes[1, 0].set_ylabel(r'$Amplitude\  (\mu V) $')
axes[1, 1].plot(x_label_freq, df_FFR_win_85_aenu_as.iloc[1, :48030], label="e vowel")  
axes[1, 1].set_xlim(0, 1300)
axes[1, 1].legend(loc="upper right")

# n vowel
axes[2, 0].plot(x_label_time, np.asarray(df_FFR_avg_85_aenu.iloc[2,:1024]))
axes[2, 0].set_xticks([])
axes[2, 0].set_ylabel(r'$Amplitude\  (\mu V) $')
axes[2, 1].plot(x_label_freq, df_FFR_win_85_aenu_as.iloc[2, :48030], label="n vowel")  
axes[2, 1].set_xlim(0, 1300)
axes[2, 1].legend(loc="upper right")

# u vowel
axes[3, 0].plot(x_label_time, np.asarray(df_FFR_avg_85_aenu.iloc[3,:1024]))
axes[3, 0].set_xlabel('Time (ms)')
axes[3, 0].set_ylabel(r'$Amplitude\  (\mu V) $')
axes[3, 1].plot(x_label_freq, df_FFR_win_85_aenu_as.iloc[3, :48030], label="u vowel")
axes[3, 1].set_xlim(0, 1300)
axes[3, 1].set_xlabel('Frequency(Hz)')
axes[3, 1].legend(loc="upper right")

fig.suptitle('grand mean 85dB 4 vowel spectral FFRs in time and frequency domain')
plt.show()
