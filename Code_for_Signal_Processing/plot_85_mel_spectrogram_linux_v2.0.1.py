...
# -*- coding: utf-8 -*-
"""
Created on Sun Oct 25 18:41:43 2020
...
def df_mel_spectrogram_txt_rename(signal_input, store_path, store_name, plot_number=88):
    # example for input
    # signal_input = df_EFR_85_aenu_retest
    # store_path = '/home/bruce/Dropbox/Project/6.Result/data_spectrogram/'
    # store_name = 'EFR_85_retest'
    ...
    for j in range(22):
        for k in range(2):
            signal_input_temp = np.array(pd.to_numeric(signal_input.iloc[2*j+k, 0:4096]))
            mel_S = librosa.feature.melspectrogram(y=signal_input_temp, sr=9606, n_fft=256, hop_length=128)
            mel_S_800 = mel_S[0:22,:]
            # shape(22,33)
            # 0 to 800Hz
            ...
            np.savetxt(store_path + store_name+ '_%s_%s.txt' % (subject_list[j], num_list[k]), mel_S_800)
...
# path for linux
df_EFR_input=pd.read_pickle('/home/bruce/Dropbox/4.Project/4.Code for Linux/df_EFR.pkl')
# path for mac
# df_EFR_input=pd.read_pickle('/Users/bruce/Dropbox/4.Project/4.Code for Linux/df_EFR.pkl')
...
mel_S = librosa.feature.melspectrogram(y=signal_in, sr=9606, n_fft=256, hop_length=128)
mel_S_800 = mel_S[0:22,:] # shape(22,33)


# test for spectrogram
spectro = librosa.core.stft(y=signal_in, center='False')
spectro = np.abs(spectro)

plt.figure()
display.specshow(librosa.power_to_db(mel_S,ref=np.max),
...
plt.figure()
display.specshow(spectro, y_axis='hz', x_axis='time')
# plt.specgram(mel_S)
plt.colorbar(format='%+2.0f dB')
...
# test for mel spectrogram 2
mel_S_2 = librosa.feature.melspectrogram(S=spectro, sr=9606)



'''
signal_in = pd.to_numeric(df_EFR_85_aenu_retest.iloc[0, 0:4096])

# Compute log Mel-filterbank energy features from an audio signal.
python_speech_features.base.logfbank(signal, 
                                     samplerate=9606, 
                                     winlen=0.025, 
                                     winstep=0.01, 
                                     nfilt=26, 
...
# save and generate the mel spectrogram [22, 9]


df_mel_spectrogram_txt_rename(signal_input=df_EFR_85_aenu_retest, 
                              store_path='/home/bruce/Dropbox/4.Project/6.Result/data_spectrogram/EFR/85_melspectrogram_22_33_rename/retest/', 
                              store_name='EFR_85_r')

df_mel_spectrogram_txt_rename(signal_input=df_EFR_85_aenu_test, 
                              store_path='/home/bruce/Dropbox/4.Project/6.Result/data_spectrogram/EFR/85_melspectrogram_22_33_rename/test/', 
                              store_name='EFR_85_t')
