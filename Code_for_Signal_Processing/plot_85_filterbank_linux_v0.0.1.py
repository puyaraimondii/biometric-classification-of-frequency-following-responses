...
# -*- coding: utf-8 -*-
"""
Created on Fri Sep  6 13:42:05 2019
...
'''
f_1, t_1, Sxx_1 = signal.spectrogram(x=pd.to_numeric(df_EFR_85_aenu_retest.iloc[0, 0:4096]), fs=9606, nperseg=128, noverlap=64, nfft=9606)

print (Sxx_1.shape)

melW=librosa.filters.mel(sr=9606,n_fft=256,n_mels=40)
melW /= np.max(melW,axis=-1)[:,None]
melX = np.dot(Sxx_1,melW.T)
...
plt.figure(2)
plt.pcolormesh(melX, cmap='inferno')
#plt.colorbar(cax, format='%+2.0f dB')
plt.ylim(0, 1300) # 2048 -> 5000hz : 533 -> 1300
plt.ylabel('Frequency [Hz]')
plt.xlabel('Time [sec]')
...
#  test for mel filterbank features
