...
def distance_mx(sig_in):
    ...
    for i in range(22):
        for j in range(22):
            temp = np.asarray(sig_in.iloc[2*i, 0:13000] - sig_in.iloc[2*j+1, 0:13000])
            temp_sum = 0
            temp_square_sum = 0
...
for i in range(176):
    #temp_aenu_FFR = df_FFR_avg_aenu_withzero.iloc[i, 0:9606]
    temp_cx = (fftpack.fft(df_FFR_avg_85_withzero.iloc[i, 0:96060])/n2)[range(int(n2/2))]
    df_cx_85 = df_cx_85.append(pd.DataFrame(temp_cx.reshape(1,48030)), ignore_index = True)
    temp_cx_win = (fftpack.fft(df_FFR_avg_win_85_withzero.iloc[i, 0:96060])/n2)[range(int(n2/2))]
    df_cx_win_85 = df_cx_win_85.append(pd.DataFrame(temp_cx_win.reshape(1,48030)), ignore_index = True)

df_cx_85 = pd.concat([df_cx_85, df_FFR_avg_85_label], axis=1) # add labels on it
df_cx_win_85 = pd.concat([df_cx_win_85, df_FFR_avg_win_85_label], axis=1) # add labels on it


# normalize df_cx_85 and df_cx_win_85 but didn't change the name
# ranging from 
test1 = df_cx_85.abs().max()
test2 = (df_cx_85.abs().max()-df_cx_85.abs().min())
df_cx_85_norm = (df_cx_85/test2)*10

df_cx_win_85_norm = ((df_cx_win_85-df_cx_win_85.mean())/(df_cx_win_85.abs().max()-df_cx_win_85.abs().min()))*10
...
df_cx_85_vsc_a = df_cx_85_vsc.iloc[0:44, :]
df_cx_85_vsc_e = df_cx_85_vsc.iloc[44:88, :]
df_cx_85_vsc_n = df_cx_85_vsc.iloc[88:132, :]
df_cx_85_vsc_u = df_cx_85_vsc.iloc[132:176, :]
df_cx_win_85_vsc_a = df_cx_win_85_vsc.iloc[0:44, :]
df_cx_win_85_vsc_e = df_cx_win_85_vsc.iloc[44:88, :]
df_cx_win_85_vsc_n = df_cx_win_85_vsc.iloc[88:132, :]
df_cx_win_85_vsc_u = df_cx_win_85_vsc.iloc[132:176, :]


# test
#fig_mag_db(df_cx_85_vsc_a, 1, title = '85dB a vowel Subject 1 in frequency domain', filename = '85_a_s1_f_domain.png')
